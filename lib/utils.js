...
export async function batchedUpdate(serviceConfig, nTriples, targetGraph,
...
export async function storeError(serviceConfig, errorMsg) {
  const id = uuid();
  ...
  const queryError = `
    ...
    INSERT DATA {
      GRAPH ${sparqlEscapeUri(serviceConfig.jobsGraph)}{
        ${sparqlEscapeUri(uri)} a ${sparqlEscapeUri(serviceConfig.errorType)}, ${sparqlEscapeUri(serviceConfig.deltaErrorType)};
          ...
          dct:creator ${sparqlEscapeUri(serviceConfig.errorCreatorUri)} .
      }
    }
  `;
...
export async function batchedQuery(serviceConfig, subjectPredicateObjectQuery,
                                    batch = 100,
