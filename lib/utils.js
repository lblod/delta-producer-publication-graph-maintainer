...
export async function batchedUpdate(nTriples, targetGraph,
  ...
  for(const chunk of chunkedArray){
    ...
    try {
      ...
      await batchedUpdate(chunk, targetGraph, operation, sleep, smallerBatch, extraHeaders, endpoint);
    }
    ++chunkCounter;
  }
}
...
export async function storeError(service_config, errorMsg) {
  ...
  const fullErrorMsg = `
    An error occured for deltaStream: "${service_config.name}".
...
//TODO: batchedQuery should be revisted.
// - Revising the code, we could combine 'batchedQuery' and 'construct query and simplify a lot of code
