...
export async function batchedUpdate( _config, nTriples, targetGraph,
                                     operation = 'INSERT', sleep = 1000,
                                     batch = 100, extraHeaders = {},
                                     endpoint = _config.MU_AUTH_ENDPOINT){
...
export async function storeError(_config, errorMsg) {
  const id = uuid();
  ...
  const queryError = `
    ...
    INSERT DATA {
      GRAPH ${sparqlEscapeUri(_config.JOBS_GRAPH)}{
        ${sparqlEscapeUri(uri)} a ${sparqlEscapeUri(_config.ERROR_TYPE)}, ${sparqlEscapeUri(_config.DELTA_ERROR_TYPE)};
          ...
          dct:creator ${sparqlEscapeUri(_config.ERROR_CREATOR_URI)} .
      }
    }
  `;
...
export async function batchedQuery( _config, subjectPredicateObjectQuery,
                                    batch = 100,
                                    sparqlEndpoint = _config.MU_AUTH_ENDPOINT,
...
export function loadConfiguration(path = '/config/export.json'){
