...
export async function batchedUpdate(service_config, nTriples, targetGraph,
                                     operation = 'INSERT', sleep = 1000,
                                     batch = 100, extraHeaders = {},
                                     endpoint = service_config.MU_AUTH_ENDPOINT){
...
export async function storeError(service_config, errorMsg) {
  ...
  const queryError = `
    ...
    INSERT DATA {
      GRAPH ${sparqlEscapeUri(service_config.JOBS_GRAPH)}{
        ${sparqlEscapeUri(uri)} a ${sparqlEscapeUri(service_config.ERROR_TYPE)}, ${sparqlEscapeUri(service_config.DELTA_ERROR_TYPE)};
          ...
          dct:creator ${sparqlEscapeUri(service_config.ERROR_CREATOR_URI)} .
      }
    }
  `;
...
export async function batchedQuery(service_config, subjectPredicateObjectQuery,
                                    batch = 100,
                                    sparqlEndpoint = service_config.MU_AUTH_ENDPOINT,
