...
export async function isTask( _config, subject ){
  ...
  const queryStr = `
   ...
   SELECT ?subject WHERE {
    GRAPH ?g {
      ...
      ?subject a ${ sparqlEscapeUri(_config.TASK_TYPE) }.
    }
   }
  `;
...
export async function loadTask( _config, subject ){
  const queryTask = `
   ${_config.PREFIXES}
   SELECT DISTINCT ?graph ?task ?id ?job ?created ?modified ?status ?index ?operation ?error WHERE {
    GRAPH ?graph {
      ...
      ?task a ${ sparqlEscapeUri(_config.TASK_TYPE) }.
...
export async function appendTaskError(_config, task, errorMsg){
  const id = uuid();
...
export async function appendGraphDatacontainerToTask(_config, task, container){
