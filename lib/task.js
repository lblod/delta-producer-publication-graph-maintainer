...
export async function isTask(serviceConfig, subject ){
  ...
  const queryStr = `
   ...
   SELECT ?subject WHERE {
    GRAPH ?g {
      ...
      ?subject a ${ sparqlEscapeUri(serviceConfig.taskType) }.
    }
   }
  `;
...
export async function loadTask(serviceConfig, subject ){
  const queryTask = `
   ${serviceConfig.prefixes}
   SELECT DISTINCT ?graph ?task ?id ?job ?created ?modified ?status ?index ?operation ?error WHERE {
    GRAPH ?graph {
      ...
      ?task a ${ sparqlEscapeUri(serviceConfig.taskType) }.
...
export async function appendTaskError(serviceConfig, task, errorMsg){
  const id = uuid();
...
export async function appendGraphDatacontainerToTask(serviceConfig, task, container){
