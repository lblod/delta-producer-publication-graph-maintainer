...
import {
    ...
    generateGetSourceTriplesQueryPage,
    generateGetCountSourceTriplesQuery,
...
export async function runHealingTask(serviceConfig, serviceExportConfig, task, isInitialSync, publishDelta) {
    ...
    try {
        ...
        for (const property of Object.keys(propertyMap)) {
            ...
            const sourceTriples = await getTriplesPaginated(serviceConfig,
                property, propertyMap,
                conceptSchemeUri,
                endpoint,
                generateGetSourceTriplesQueryPage, generateGetCountSourceTriplesQuery);

            endpoint = serviceConfig.useVirtuosoForExpensiveSelects ?
                PUBLICATION_VIRTUOSO_ENDPOINT : PUBLICATION_MU_AUTH_ENDPOINT;
            const publicationGraphTriples = await getTriplesPaginated(serviceConfig,
                property, propertyMap,
                conceptSchemeUri,
                endpoint,
                generateGetSourceTriplesQueryPage, generateGetCountSourceTriplesQuery);
 ...
 * Gets the triples for a property
 */
async function getTriplesPaginated(serviceConfig, property, propertyMap, conceptSchemeUri, endpoint, generateQueryPage, generateCountQuery) {
    let sourceTriples = tmp.fileSync();
    ...
    for (const config of propertyMap[property]) {
        ...
        let count = 0;
        let countQueryStr = generateCountQuery(
            {
                config,
                property,
                publicationGraph: serviceConfig.publicationGraph,
        ...
        const countResult = await query(countQueryStr, {}, { sparqlEndpoint: endpoint, mayRetry: true });
        if (countResult.results.bindings?.length) {
        ...
        for (let currentPage = 1; currentPage <= totalPages; currentPage++) {
            ...
            response.push(...triples);
        }
        let scopedSourceTriplesFile = arrayToFile(response, tmp.fileSync());
        const diffs = diffFiles(scopedSourceTriplesFile, sourceTriples);
